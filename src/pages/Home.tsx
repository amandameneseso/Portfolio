...
const Notifications = [
  <>
    Notificação: <strong>Modo silencioso detectado.</strong> Clique em qualquer
    lugar da tela para liberar os sons.
  </>,
  <>
    Notícia: "<b>Conferência global</b>:{" "}
    <i>
      Programadores debatem se React foi criado para testar a paciência humana.
    </i>
    "
  </>,
  <>
    Espaço de armazenamento: "Seu disco local está quase cheio. Por favor,
    delete arquivos inúteis (como as 50 versões do mesmo projeto)."
  </>,
  <>
    Notícia: "<b>Programadora informa:</b>{" "}
    <i>
      “Se houver algum bug nesse portfólio, a culpa é do gato — ele passou pelo
      teclado”.
    </i>
    "
  </>,
  <>
    WhatsApp: "Você tem 347 novas mensagens do grupo 'como centralizar uma
    div'."
  </>,
  <>
    Notícia: "<b>Estudo mostra</b>:{" "}
    <i>Quem comenta o próprio código vive 10 anos a mais.</i>"
  </>,
  <>
    LinkedIn: "Seu portfólio foi visto por um recrutador. Mas ele só queria ver
    as notícias."
  </>,
  <>
    Notícia: "<b>Inteligência Artificial admite:</b>{" "}
    <i>“Sim, eu também uso Stack Overflow”.</i>"
  </>,
  <>
    Notícia: "<b>Falha no espaço-tempo:</b>{" "}
    <i>Devs descobrem que “só mais um commit” pode durar 5 horas.</i>"
  </>,
  <>
  ...
  <>
    Notícia: "<b>Escândalo global:</b>{" "}
    <i>
      Documentação completa e perfeita é encontrada. Programadores não sabem
      como reagir.
    </i>
    "
  </>,
  ...
  <>
    Notícia: "<b>Economia em alerta:</b>{" "}
    <i>Preço do café triplica após devs admitirem dependência extrema.</i>"
  </>,
  <>
    Notícia: "<b>Novo app revolucionário:</b>{" "}
    <i>Envia notificações para lembrar você de ignorar notificações.</i>"
  </>,
  <>
    Notícia: "<b>Conflito Internacional:</b>{" "}
    <i>
      Programadores usuários de Windows e Linux declaram trégua para debater a
      melhor forma de centralizar uma div.
    </i>
    "
  </>,
  <>Notícia: "Escada impede fuga de robô rolante."</>,
  <>
    Notícia: "<b>Descoberta energia barata e limpa:</b>{" "}
    <i>
      Empresa de tecnologia afirma ter desenvolvido nova fonte de energia que
      funciona totalmente a base de gargalhadas de crianças. Nega relatos da
      existência de monstros por perto.
    </i>
    "
  </>,
...
function Home() {
  ...
  useEffect(() => {
    ...
    if (areNotificationsEnabled) {
      if (notificationIndex === 0 && Notifications.length > 0) {
        // 1. Disparo Inicial (com 2 segundos de atraso)
        timerId = window.setTimeout(() => {
          setNotifications((prev) => [...prev, Notifications[0]]);
          setNotificationIndex((prev) => prev + 1); // Dispara a re-execução para configurar o INTERVAL
        }, 2000); // Atraso de 2 segundos
      } else if (
        notificationIndex > 0 &&
        notificationIndex < Notifications.length
      ) {
        // 2. Disparo Recorrente (a cada 30 segundos)
        timerId = window.setInterval(() => {
          const message = Notifications[notificationIndex];
          setNotifications((prev) => [...prev, message]);
          setNotificationIndex((prev) => prev + 1); // Incrementa o índice para o próximo ciclo
        }, 30000); // Intervalo de 30 segundos
      }
    }

    return () => {
      // Garante que o timer seja limpo
      if (timerId !== undefined) {
        window.clearTimeout(timerId); // Limpa o setTimeout, caso seja o timer inicial
        window.clearInterval(timerId); // Limpa o setInterval, caso seja o timer recorrente
      }
    };
